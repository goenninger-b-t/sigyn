...
(defmethod client-structure-write ((object sapnwrfc-object) rfc-container-handle &optional (slot-rfc-kind (list :importing :changing)))
...
(defmethod client-structure-read ((object sapnwrfc-object) rfc-container-handle &optional (slot-rfc-kind (list :exporting :changing)))
  (let ((slot-kinds (alexandria:ensure-list slot-rfc-kind)))
    (loop for class in (closer-mop:class-precedence-list (class-of object))
	  do
	     (loop for slot in (closer-mop:class-direct-slots class)
		   when (typep slot 'sapnwrfc-object-slot)
			...
			(let ((rfc-type       (sapnwrfc-object-slot-rfc-type slot))
			      ...
			      (rfc-param-name (sapnwrfc-object-slot-rfc-param-name slot))
			      (rfc-kind       (sapnwrfc-object-slot-rfc-kind slot)))
			  (let ((value (if (member rfc-kind slot-kinds)
					   (param-get rfc-container-handle rfc-type rfc-param-name rfc-length))))
