...
(declaim (inline rfc-create-server))
(defun rfc-create-server (server-connection-parameter-list)
  (with-rfc-connection-parameters (conn-param-ptr nr-params server-connection-parameter-list)
    (with-rfc-error-info (error-info-ptr)
      (let ((server-handle (%rfc-create-server conn-param-ptr nr-params error-info-ptr)))
	(log:debug "Creating RFC Server: Server handle = ~S" server-handle)
	(if (cffi:null-pointer-p server-handle)
	    (check-and-handle-rfc-error-info error-info-ptr))
	server-handle))))

(declaim (inline rfc-destroy-server))
(defun rfc-destroy-server (server-handle)
  (with-rfc-error-info (error-info-ptr)
    (let ((rc (%rfc-destroy-server server-handle error-info-ptr)))
      (check-and-handle-rfc-error-info error-info-ptr :on-error :ignore)
      rc)))

(declaim (inline rfc-launch-server))
(defun rfc-launch-server (server-handle)
  (with-rfc-error-info (error-info-ptr)
    (let ((rc (%rfc-launch-server server-handle error-info-ptr)))
      (check-and-handle-rfc-error-info error-info-ptr)
      rc)))

(declaim (inline rfc-shutdown-server))
(defun rfc-shutdown-server (server-handle timeout)
  (with-rfc-error-info (error-info-ptr)
